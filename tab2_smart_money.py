...
def _inject_popup_css():
    ...
    <style>
    #sp-rail {
        position: fixed !important; top: 16px !important; left: 0 !important;
        width: 100vw !important; z-index: 2147483647 !important;
        pointer-events: none !important; display: flex !important;
        flex-direction: column !important; align-items: flex-start !important;
        gap: 8px !important; padding-left: 16px !important;
    }
    .sp-box {
        pointer-events: auto !important; display: inline-flex !important;
        align-items: center !important; gap: 10px !important;
        padding: 10px 20px 10px 14px !important; border-radius: 5px !important;
        border-left: 5px solid !important;
        font-family: 'Barlow Condensed', 'JetBrains Mono', monospace !important;
        font-size: 14px !important; font-weight: 700 !important;
        letter-spacing: 1.3px !important; white-space: nowrap !important;
    ...
    @keyframes sp-slide { 0% { opacity:0; transform:translateX(-110%); } 100% { opacity:1; transform:translateX(0); } }
    @keyframes sp-fade  { 0%,70% { opacity:1; } 100% { opacity:0; } }
    ...
    .sp-icon { font-size:17px !important; } .sp-msg { font-size:14px !important; }
    </style>
    <div id="sp-rail"></div>
    <script>
    (function(){ var r=document.getElementById('sp-rail');
      if(r&&r.parentNode!==document.body){document.body.appendChild(r);}
      if(!r){r=document.createElement('div');r.id='sp-rail';document.body.appendChild(r);} })();
    </script>
...
def _show_popup(message: str, kind: str = "danger", icon: str = "🚨", duration_ms: int = 5500):
    ...
    (function(){{
      ...
      if(!rail){{rail=document.createElement('div');rail.id='sp-rail';
        Object.assign(rail.style,{{position:'fixed',top:'16px',left:'0',width:'100vw',
          zIndex:'2147483647',pointerEvents:'none',display:'flex',flexDirection:'column',
          alignItems:'flex-start',gap:'8px',paddingLeft:'16px'}});document.body.appendChild(rail);}}
      else if(rail.parentNode!==document.body){{document.body.appendChild(rail);}}
      var p=document.createElement('div');p.id='{uid}';p.className='sp-box sp-{kind}';
      p.innerHTML='<span class="sp-icon">{icon}</span><span class="sp-msg">{safe_msg}</span>';
      rail.insertBefore(p,rail.firstChild);
      setTimeout(function(){{var el=document.getElementById('{uid}');
        if(el&&el.parentNode)el.parentNode.removeChild(el);}},{duration_ms});
    }})();
...
def render():
    ...
    if prev_gex > 0 and total_net_gex < 0:
        _show_popup("GEX FLIP TO TRENDING MODE", kind="danger", icon="🔴")
        st.session_state.alert_log.append({"time": datetime.now(IST).strftime("%H:%M:%S"), "msg": "GEX Flip Trending", "type": "DANGER"})
    elif prev_gex < 0 and total_net_gex > 0:
        _show_popup("GEX FLIP TO MEAN REVERSION MODE", kind="success", icon="🟢")
        st.session_state.alert_log.append({"time": datetime.now(IST).strftime("%H:%M:%S"), "msg": "GEX Flip Mean Rev", "type": "INFO"})
    ...
    try:
        ...
        gex_fig.add_bar(x=gex_sorted_chart["Strike"], y=gex_sorted_chart["GEX"]/1e6, marker_color=colors_gex, name="GEX")
        ...
        gex_fig.update_layout(height=300, paper_bgcolor="#070b0f", plot_bgcolor="#070b0f",
                               font=dict(family="JetBrains Mono", color="#7fa8c8", size=11),
                               margin=dict(l=40, r=20, t=20, b=40),
                               xaxis=dict(gridcolor="#1e3040", tickangle=-45, tickformat="d"),
                               yaxis=dict(gridcolor="#1e3040", title="GEX (Millions)"), showlegend=False)
    ...
    if "CE_OI_Change" in oc2.columns and "PE_OI_Change" in oc2.columns:
        ce_oi_chg2 = float(atm_row2["CE_OI_Change"]); pe_oi_chg2 = float(atm_row2["PE_OI_Change"])
    ...
    if atm_s2 in strikes2:
        ...
        for _, row2 in mini2.iterrows():
            ce_c = sum([row2["CE_OI_Change"]>row2["PE_OI_Change"], row2["CE_Volume"]>row2["PE_Volume"], row2["CE_OI"]>row2["PE_OI"]])
            ce_s_list.append(ce_c); pe_s_list.append(3 - ce_c)
        mini2["CE_Strength"] = ce_s_list; mini2["PE_Strength"] = pe_s_list
        show2 = [c for c in ["Strike","CE_LTP","CE_OI","CE_OI_Change","CE_Strength","PE_Strength","PE_OI_Change","PE_OI","PE_LTP","GEX"] if c in mini2.columns]
    ...
    try:
        ...
        hm_fig = go.Figure()
        hm_fig.add_bar(x=oc_hm["Strike"], y=oc_hm["CE_OI"]/1e5, name="CE OI", marker_color="#ff3d57", opacity=0.85)
        hm_fig.add_bar(x=oc_hm["Strike"], y=oc_hm["PE_OI"]/1e5, name="PE OI", marker_color="#00e676", opacity=0.85)
        hm_fig.update_layout(barmode="group", height=320, paper_bgcolor="#070b0f", plot_bgcolor="#070b0f",
                              font=dict(family="JetBrains Mono", color="#7fa8c8", size=11),
                              margin=dict(l=40, r=20, t=20, b=40),
                              xaxis=dict(gridcolor="#1e3040", tickangle=-45, tickformat="d"),
    ...
    st.markdown("---")
    section_header("CE vs PE OI per Strike", "OI delta chart + strike-wise Net Bias")
    try:
        oc_cmp = oc2.copy()
        if all(c in oc_cmp.columns for c in ["CE_OI", "PE_OI", "CE_OI_Change", "PE_OI_Change"]):
            ...
            tab_oi_delta, tab_oi_total = st.tabs(["OI Change (Delta) by Strike", "Total CE OI vs PE OI"])
            with tab_oi_delta:
                cmp_fig = go.Figure()
                ...
                cmp_fig.add_bar(x=oc_cmp["Strike"], y=oc_cmp["CE_OI_Change"]/1e3, name="CE OI D", marker_color=ce_colors, opacity=0.9,
                                text=[f"{v/1e3:+.1f}K" for v in oc_cmp["CE_OI_Change"]], textposition="outside", textfont=dict(size=9, color="#ff3d57"))
                cmp_fig.add_bar(x=oc_cmp["Strike"], y=oc_cmp["PE_OI_Change"]/1e3, name="PE OI D", marker_color=pe_colors, opacity=0.9,
                                text=[f"{v/1e3:+.1f}K" for v in oc_cmp["PE_OI_Change"]], textposition="outside", textfont=dict(size=9, color="#00e676"))
                cmp_fig.add_vline(x=float(atm_cmp), line_color="#00d4ff", line_dash="dot", line_width=1.5,
                                  annotation_text="ATM", annotation_font_color="#00d4ff", annotation_font_size=10)
                cmp_fig.add_hline(y=0, line_color="#3a6080", line_width=0.8)
                cmp_fig.update_layout(barmode="group", height=320, paper_bgcolor="#070b0f", plot_bgcolor="#070b0f",
                                      font=dict(family="JetBrains Mono", color="#7fa8c8", size=11),
                                      margin=dict(l=40, r=20, t=20, b=40),
            ...
            with tab_oi_total:
                tot_fig = go.Figure()
                tot_fig.add_bar(x=oc_cmp["Strike"], y=oc_cmp["CE_OI"]/1e5, name="CE OI", marker_color="#ff3d57", opacity=0.85)
                tot_fig.add_bar(x=oc_cmp["Strike"], y=oc_cmp["PE_OI"]/1e5, name="PE OI", marker_color="#00e676", opacity=0.85)
                tot_fig.add_vline(x=float(atm_cmp), line_color="#00d4ff", line_dash="dot", line_width=1.5,
                                  annotation_text="ATM", annotation_font_color="#00d4ff", annotation_font_size=10)
                tot_fig.update_layout(barmode="group", height=320, paper_bgcolor="#070b0f", plot_bgcolor="#070b0f",
                                      font=dict(family="JetBrains Mono", color="#7fa8c8", size=11),
                                      margin=dict(l=40, r=20, t=20, b=40),
                                      xaxis=dict(gridcolor="#1e3040", tickangle=-45, tickformat="d", title="Strike"),
                                      yaxis=dict(gridcolor="#1e3040", title="OI (Lakhs)"),
            ...
            def _net_bias_oi(ce_oi, pe_oi, ce_chg, pe_chg):
                ce_oi = abs(float(ce_oi)); pe_oi = abs(float(pe_oi)); total_oi = ce_oi + pe_oi
                if total_oi == 0: return "NEUTRAL"
                oi_diff_pct = abs(ce_oi - pe_oi) / max(total_oi, 1) * 100
                if oi_diff_pct < 15: return "NEUTRAL"
                return "BEARISH" if ce_oi > pe_oi else "BULLISH"
            ...
            for _, r in oc_cmp.iterrows():
                ...
                ce_oi_chg = float(r["CE_OI_Change"]); pe_oi_chg = float(r["PE_OI_Change"])
                total_oi = ce_oi + pe_oi
                rows_cmp.append({
                    "Strike": f"{int(r['Strike']):,}{atm_tag}",
                    "CE OI": f"{ce_oi/1e5:.2f}L", "CE OI D": f"{ce_oi_chg/1e3:+.1f}K",
                    "CE %": f"{ce_oi/max(total_oi,1)*100:.0f}%", "PE %": f"{pe_oi/max(total_oi,1)*100:.0f}%",
                    "PE OI D": f"{pe_oi_chg/1e3:+.1f}K", "PE OI": f"{pe_oi/1e5:.2f}L",
                    "Money": money, "Net Bias": _net_bias_oi(ce_oi, pe_oi, ce_oi_chg, pe_oi_chg),
                })

            df_cmp = pd.DataFrame(rows_cmp)
            total_ce_cmp = oc_cmp["CE_OI_Change"].sum(); total_pe_cmp = oc_cmp["PE_OI_Change"].sum()
            total_ce_oi  = oc_cmp["CE_OI"].sum();        total_pe_oi  = oc_cmp["PE_OI"].sum()
            ...
            with mc1:
                st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid #ff3d57;border-radius:3px;padding:10px 14px;">
                    <div style="font-family:'Barlow Condensed',sans-serif;font-size:9px;letter-spacing:2px;color:#7fa8c8;">TOTAL CE OI D</div>
                    <div style="font-family:'JetBrains Mono',monospace;font-size:18px;font-weight:700;color:{'#ff3d57' if total_ce_cmp>0 else '#00e676'};">{total_ce_cmp/1e3:+.0f}K</div></div>""", unsafe_allow_html=True)
            with mc2:
                st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid #00e676;border-radius:3px;padding:10px 14px;">
                    <div style="font-family:'Barlow Condensed',sans-serif;font-size:9px;letter-spacing:2px;color:#7fa8c8;">TOTAL PE OI D</div>
                    <div style="font-family:'JetBrains Mono',monospace;font-size:18px;font-weight:700;color:{'#00e676' if total_pe_cmp>0 else '#ff3d57'};">{total_pe_cmp/1e3:+.0f}K</div></div>""", unsafe_allow_html=True)
            with mc3:
                pcr_c = "#00e676" if pcr_cmp > 1.2 else "#ff3d57" if pcr_cmp < 0.8 else "#ffd600"
                st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid {pcr_c};border-radius:3px;padding:10px 14px;">
                    <div style="font-family:'Barlow Condensed',sans-serif;font-size:9px;letter-spacing:2px;color:#7fa8c8;">PCR (OI)</div>
                    <div style="font-family:'JetBrains Mono',monospace;font-size:18px;font-weight:700;color:{pcr_c};">{pcr_cmp:.2f}</div></div>""", unsafe_allow_html=True)
            with mc4:
                st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid #ffd600;border-radius:3px;padding:10px 14px;">
                    <div style="font-family:'Barlow Condensed',sans-serif;font-size:9px;letter-spacing:2px;color:#7fa8c8;">STRIKE BIAS COUNT</div>
                    <div style="font-family:'JetBrains Mono',monospace;font-size:14px;font-weight:700;color:#ffd600;margin-top:3px;">B:{bull_count} BR:{bear_count} N:{neut_count}</div></div>""", unsafe_allow_html=True)
            with mc5:
                st.markdown(f"""<div style="background:#0d1117;border:1px solid {ob_color};border-left:3px solid {ob_color};border-radius:3px;padding:10px 14px;">
            ...
            <div style="display:flex;height:8px;border-radius:4px;overflow:hidden;margin-bottom:12px;">
              ...
              <div style="width:{pe_pct_split:.1f}%;background:#00e676;"></div></div>""", unsafe_allow_html=True)
            st.dataframe(df_cmp, use_container_width=True, height=min(35*len(df_cmp)+38, 550))
        else:
    ...
    # ======================================================
    # OI SPIKE CHANGE DETECTOR
    # ======================================================
    st.markdown("---")
    section_header("OI Spike Change Detector", "Strikes with sudden large OI build-up or unwinding")
    try:
        oc_ois = oc2.copy()
        if all(c in oc_ois.columns for c in ["CE_OI_Change", "PE_OI_Change", "CE_OI", "PE_OI"]):
            ...
            for _, r in ce_oi_spikes.iterrows():
                ...
                if mult_oi >= 3.0:
                    _show_popup(f"CE OI SPIKE {int(r['Strike'])} - {direction} ({mult_oi:.1f}x avg)", kind="danger", icon="🔴")
                    if "alert_log" in st.session_state:
                        st.session_state.alert_log.append({"time": datetime.now(IST).strftime("%H:%M:%S"),
                            "msg": f"CE OI Spike {int(r['Strike'])} {direction} ({mult_oi:.1f}x)", "type": "DANGER"})
            for _, r in pe_oi_spikes.iterrows():
                ...
                if mult_oi >= 3.0:
                    _show_popup(f"PE OI SPIKE {int(r['Strike'])} - {direction} ({mult_oi:.1f}x avg)", kind="success", icon="🟢")
                    if "alert_log" in st.session_state:
                        st.session_state.alert_log.append({"time": datetime.now(IST).strftime("%H:%M:%S"),
                            "msg": f"PE OI Spike {int(r['Strike'])} {direction} ({mult_oi:.1f}x)", "type": "INFO"})

            def _detect_strike_continue(df_sorted, col_chg, threshold_mult, mean_chg):
                runs = []; current_run = []; prev_dir = None
                for _, row_sc in df_sorted.iterrows():
                    chg = float(row_sc[col_chg])
                    if abs(chg) < mean_chg * 0.5:
                        if len(current_run) >= 2: runs.append(current_run[:])
                        current_run = []; prev_dir = None; continue
                    direction = "UP" if chg > 0 else "DOWN"
                    if direction == prev_dir:
                        current_run.append({"strike": int(row_sc["Strike"]), "chg": chg, "dir": direction})
                    else:
                        if len(current_run) >= 2: runs.append(current_run[:])
                        current_run = [{"strike": int(row_sc["Strike"]), "chg": chg, "dir": direction}]; prev_dir = direction
                if len(current_run) >= 2: runs.append(current_run)
                return runs
            ...
            with oi_sc1:
                st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid #ff3d57;border-radius:3px;padding:10px 14px;">
                    ...
                    <div style="font-family:'JetBrains Mono',monospace;font-size:24px;font-weight:700;color:#ff3d57;">{len(ce_oi_spikes)}</div>
                    <div style="font-size:10px;color:#3a6080;">{oi_spike_mult:.0f}x avg OI change</div></div>""", unsafe_allow_html=True)
            with oi_sc2:
                st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid #00e676;border-radius:3px;padding:10px 14px;">
                    <div style="font-family:'Barlow Condensed',sans-serif;font-size:9px;letter-spacing:2px;color:#7fa8c8;">PE OI SPIKES</div>
                    <div style="font-family:'JetBrains Mono',monospace;font-size:24px;font-weight:700;color:#00e676;">{len(pe_oi_spikes)}</div>
                    <div style="font-size:10px;color:#3a6080;">{oi_spike_mult:.0f}x avg OI change</div></div>""", unsafe_allow_html=True)
            with oi_sc3:
                ce_continues = len([r for r in ce_runs if len(r) >= 2]); best_ce_run = max((len(r) for r in ce_runs), default=0)
                st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid #ff8c00;border-radius:3px;padding:10px 14px;">
                    ...
                    <div style="font-size:10px;color:#3a6080;">Longest: {best_ce_run} strikes</div></div>""", unsafe_allow_html=True)
            with oi_sc4:
                pe_continues = len([r for r in pe_runs if len(r) >= 2]); best_pe_run = max((len(r) for r in pe_runs), default=0)
                st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid #00d4ff;border-radius:3px;padding:10px 14px;">
                    ...
                    <div style="font-size:10px;color:#3a6080;">Longest: {best_pe_run} strikes</div></div>""", unsafe_allow_html=True)
                ...
                with sc_tab1:
                    if ce_runs:
                        ...
                        for run in sorted(ce_runs, key=lambda x: -len(x)):
                            ce_run_rows.append({"CE Strikes": f"{run[0]['strike']} -> {run[-1]['strike']}", "Count": len(run),
                                "Direction": "BUILD-UP" if run[0]["dir"]=="UP" else "UNWIND",
                                "Total OI D": f"{sum(r['chg'] for r in run)/1e3:+.1f}K",
                                "Strength": "STRONG" if len(run)>=3 else "MODERATE",
                                "Meaning": "Resistance building" if run[0]["dir"]=="UP" else "CE Unwinding (Bullish)"})
                        st.dataframe(pd.DataFrame(ce_run_rows), use_container_width=True, height=min(35*len(ce_run_rows)+38, 320))
                    else:
                ...
                with sc_tab2:
                    if pe_runs:
                        ...
                        for run in sorted(pe_runs, key=lambda x: -len(x)):
                            pe_run_rows.append({"PE Strikes": f"{run[0]['strike']} -> {run[-1]['strike']}", "Count": len(run),
                                "Direction": "BUILD-UP" if run[0]["dir"]=="UP" else "UNWIND",
                                "Total OI D": f"{sum(r['chg'] for r in run)/1e3:+.1f}K",
                                "Strength": "STRONG" if len(run)>=3 else "MODERATE",
                                "Meaning": "Support building" if run[0]["dir"]=="UP" else "PE Unwinding (Bearish)"})
                        st.dataframe(pd.DataFrame(pe_run_rows), use_container_width=True, height=min(35*len(pe_run_rows)+38, 320))
                    else:
            ...
            for strike_val in sorted(all_spike_strikes):
                ...
                row_s = row_s.iloc[0]
                ce_chg_s = float(row_s["CE_OI_Change"]); pe_chg_s = float(row_s["PE_OI_Change"])
                ce_mult_s = abs(ce_chg_s) / max(ce_oi_mean, 1); pe_mult_s = abs(pe_chg_s) / max(pe_oi_mean, 1)
                ...
                oi_spike_rows.append({
                    "Strike": f"{int(strike_val):,}{atm_tag}", "Money": money,
                    "CE OI D": f"{ce_chg_s/1e3:+.1f}K",
                    "CE Dir": f"{'R' if ce_chg_s>0 else 'G'} {'INC' if ce_chg_s>0 else 'DEC'}",
                    "CE Spike": f"{ce_mult_s:.1f}x" if is_ce_spike else "-",
                    "PE OI D": f"{pe_chg_s/1e3:+.1f}K",
                    "PE Dir": f"{'G' if pe_chg_s>0 else 'R'} {'INC' if pe_chg_s>0 else 'DEC'}",
                    "PE Spike": f"{pe_mult_s:.1f}x" if is_pe_spike else "-",
                    "Signal": "BEARISH" if (is_ce_spike and ce_chg_s>0 and ce_chg_s>pe_chg_s)
                              else "BULLISH" if (is_pe_spike and pe_chg_s>0 and pe_chg_s>ce_chg_s) else "WATCH",
                })
            if oi_spike_rows:
                st.dataframe(pd.DataFrame(oi_spike_rows), use_container_width=True, height=min(35*len(oi_spike_rows)+38, 420))

            oi_spk_fig = go.Figure()
            oc_ois_ch = oc_ois.sort_values("Strike")
            oi_spk_fig.add_bar(x=oc_ois_ch["Strike"], y=oc_ois_ch["CE_OI_Change"]/1e3, name="CE OI D",
                                marker_color=["#ff1744" if abs(v)>=ce_oi_mean*oi_spike_mult else "#ff3d57" if v>0 else "#ff8a80" for v in oc_ois_ch["CE_OI_Change"]], opacity=0.9)
            oi_spk_fig.add_bar(x=oc_ois_ch["Strike"], y=oc_ois_ch["PE_OI_Change"]/1e3, name="PE OI D",
                                marker_color=["#00c853" if abs(v)>=pe_oi_mean*oi_spike_mult else "#00e676" if v>0 else "#69f0ae" for v in oc_ois_ch["PE_OI_Change"]], opacity=0.9)
            oi_spk_fig.add_vline(x=float(atm_ois), line_color="#00d4ff", line_dash="dot", line_width=1.5,
                                  annotation_text="ATM", annotation_font_color="#00d4ff", annotation_font_size=10)
            oi_spk_fig.update_layout(barmode="group", height=340, paper_bgcolor="#070b0f", plot_bgcolor="#070b0f",
                                      font=dict(family="JetBrains Mono", color="#7fa8c8", size=11),
                                      margin=dict(l=40, r=20, t=20, b=40),
                                      xaxis=dict(gridcolor="#1e3040", tickangle=-45, tickformat="d", title="Strike"),
                                      yaxis=dict(gridcolor="#1e3040", title="OI Change (000s)"),
    ...
    # ======================================================
    # VOLUME SPIKE ALERTS  -  WITH PERSISTENT TIME TRACKING
    # ======================================================
    st.markdown("---")
    section_header("Volume Spike Alerts", "Strikes where CE or PE volume is 2x or more above average - unusual institutional activity")
    try:
        oc_vs = oc2.copy()
        if "CE_Volume" in oc_vs.columns and "PE_Volume" in oc_vs.columns:
            ...
            def _spike_bias(ce_chg, pe_chg):
                ce_abs = abs(float(ce_chg)); pe_abs = abs(float(pe_chg)); total = ce_abs + pe_abs
                if total == 0: return "NEUTRAL"
                diff_pct = abs(ce_abs - pe_abs) / max(total, 1) * 100
                if diff_pct < 10: return "NEUTRAL"
                return "BEARISH" if ce_abs > pe_abs else "BULLISH"

            # ── Persistent timestamp tracker ──────────────────────────────────
            # Stored in st.session_state.vol_spike_times
            # Key  : ("CE"|"PE", strike_int)
            # Value: {"time": "HH:MM:SS", "prev_mult": float, "prev_bias": str}
            # Rule : timestamp is set on FIRST detection; refreshed only when
            #        mult changes >= 0.3x OR bias flips.
            if "vol_spike_times" not in st.session_state:
                st.session_state.vol_spike_times = {}

            now_str     = datetime.now(IST).strftime("%H:%M:%S")
            active_keys = set()

            def _record(opt_type: str, strike: int, mult: float, bias: str):
                key  = (opt_type, strike)
                prev = st.session_state.vol_spike_times.get(key)
                if prev is None:
                    st.session_state.vol_spike_times[key] = {"time": now_str, "prev_mult": mult, "prev_bias": bias}
                elif abs(mult - prev["prev_mult"]) >= 0.3 or bias != prev["prev_bias"]:
                    st.session_state.vol_spike_times[key] = {"time": now_str, "prev_mult": mult, "prev_bias": bias}

            def _get_time(opt_type: str, strike: int) -> str:
                entry = st.session_state.vol_spike_times.get((opt_type, strike))
                return entry["time"] if entry else now_str
            ...
            for _, r in ce_spikes.iterrows():
                ...
                bias   = _spike_bias(ce_chg, pe_chg)
                _record("CE", int(r["Strike"]), mult, bias)
                active_keys.add(("CE", int(r["Strike"])))
                spike_alerts.append({
                    "Type":     "CE",
                    "Strike":   int(r["Strike"]),
                    "Volume":   f"{int(r['CE_Volume']):,}",
                    "vs Avg":   f"{mult:.1f}x",
                    "CE OI D":  f"{ce_chg/1e3:+.1f}K",
                    "PE OI D":  f"{pe_chg/1e3:+.1f}K",
                    "Money":    rel,
                    "Net Bias": bias,
                    "Time":     _get_time("CE", int(r["Strike"])),
                })
            for _, r in pe_spikes.iterrows():
                ...
                ce_chg = r.get("CE_OI_Change", 0); pe_chg = r.get("PE_OI_Change", 0)
                bias   = _spike_bias(ce_chg, pe_chg)
                _record("PE", int(r["Strike"]), mult, bias)
                active_keys.add(("PE", int(r["Strike"])))
                spike_alerts.append({
                    "Type":     "PE",
                    "Strike":   int(r["Strike"]),
                    ...
                    "Money":    rel,
                    "Net Bias": bias,
                    "Time":     _get_time("PE", int(r["Strike"])),
                })

            # Prune stale keys
            for k in [k for k in st.session_state.vol_spike_times if k not in active_keys]:
                del st.session_state.vol_spike_times[k]

            if spike_alerts:
                df_spikes = pd.DataFrame(spike_alerts).sort_values("Strike")

                # Toast for high spikes
                high_spikes = [a for a in spike_alerts if float(a["vs Avg"].replace("x","")) >= 3.0]
                for hs in high_spikes[:3]:
                    _show_popup(f"Vol Spike {hs['Type']} {hs['Strike']} - {hs['vs Avg']} avg", kind="warning", icon="🔔")
                    if "alert_log" in st.session_state:
                        st.session_state.alert_log.append({"time": now_str,
                            "msg": f"Vol Spike {hs['Type']} {hs['Strike']} ({hs['vs Avg']})", "type": "DANGER"})

                # OI bias
                total_ce_oi_vs = float(oc_vs["CE_OI"].sum()); total_pe_oi_vs = float(oc_vs["PE_OI"].sum())
                total_oi_sum   = total_ce_oi_vs + total_pe_oi_vs
                oi_diff_pct    = abs(total_ce_oi_vs - total_pe_oi_vs) / max(total_oi_sum, 1) * 100
                if oi_diff_pct < 10:   oi_bias, oi_bias_color = "NEUTRAL", "#7fa8c8"
                elif total_ce_oi_vs > total_pe_oi_vs: oi_bias, oi_bias_color = "BEARISH", "#ff3d57"
                else:                  oi_bias, oi_bias_color = "BULLISH", "#00e676"
                ...
                with sv1:
                    st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid #ff3d57;border-radius:3px;padding:10px 14px;">
                        ...
                        <div style="font-size:10px;color:#3a6080;">CE Spikes: {len(ce_spikes)} strikes</div></div>""", unsafe_allow_html=True)
                with sv2:
                    st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid #00e676;border-radius:3px;padding:10px 14px;">
                        ...
                        <div style="font-size:10px;color:#3a6080;">PE Spikes: {len(pe_spikes)} strikes</div></div>""", unsafe_allow_html=True)
                with sv3:
                    st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid {oi_bias_color};border-radius:3px;padding:10px 14px;">
                        ...
                        <div style="font-size:10px;color:#3a6080;">Diff: {oi_diff_pct:.1f}% {'> 10%' if oi_diff_pct>=10 else '< 10% Neutral'}</div></div>""", unsafe_allow_html=True)
                with sv4:
                    ...
                    st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid {pcr_c};border-radius:3px;padding:10px 14px;">
                        ...
                        <div style="font-size:10px;color:#3a6080;">PE/CE OI ratio</div></div>""", unsafe_allow_html=True)
                with sv5:
                    top_spike = max(spike_alerts, key=lambda x: float(x["vs Avg"].replace("x","")))
                    st.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-left:3px solid #ffd600;border-radius:3px;padding:10px 14px;">
                        <div style="font-family:'Barlow Condensed',sans-serif;font-size:9px;letter-spacing:2px;color:#7fa8c8;">BIGGEST SPIKE</div>
                        <div style="font-family:'JetBrains Mono',monospace;font-size:16px;font-weight:700;color:#ffd600;">{top_spike['Type']} {top_spike['Strike']}</div>
                ...
                <div style="display:flex;height:8px;border-radius:4px;overflow:hidden;margin-bottom:12px;">
                  ...
                  <div style="width:{pe_pct_bar:.1f}%;background:#00e676;"></div></div>""", unsafe_allow_html=True)

                st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)

                # ── Main spike table (Net Bias + Time on the right) ──────────
                st.dataframe(
                    df_spikes,
                    ...
                    column_config={
                        ...
                        "Volume":   st.column_config.TextColumn("Volume",   width=115),
                        "vs Avg":   st.column_config.TextColumn("vs Avg",   width=65),
                        "CE OI D":  st.column_config.TextColumn("CE OI D", width=85),
                        "PE OI D":  st.column_config.TextColumn("PE OI D", width=85),
                ...
                # ── Strike Tracker panel — right-side compact cards ───────────
                st.markdown("""<div style="font-family:'Barlow Condensed',sans-serif;font-size:11px;
                letter-spacing:1.8px;color:#ff8c00;margin:16px 0 8px 0;">
                SPIKE TRACKER  -  STRIKE  |  NET BIAS  |  DETECTED AT</div>""", unsafe_allow_html=True)

                cards_html = '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:16px;">'
                for alert in sorted(spike_alerts, key=lambda x: x["Strike"]):
                    bias   = alert["Net Bias"]
                    t_str  = alert["Time"]
                    strike = alert["Strike"]
                    opt    = alert["Type"]
                    ...
                    if "BULLISH" in bias:
                        border_c, bias_c, bg_c = "#00e676", "#69f0ae", "rgba(3,20,10,0.97)"
                        bias_icon = "BULLISH"
                    elif "BEARISH" in bias:
                        border_c, bias_c, bg_c = "#ff3d57", "#ff6b6b", "rgba(28,4,4,0.97)"
                        bias_icon = "BEARISH"
                    else:
                        border_c, bias_c, bg_c = "#7fa8c8", "#a0c4d8", "rgba(5,12,20,0.97)"
                        bias_icon = "NEUTRAL"

                    opt_dot = "🔴" if opt == "CE" else "🟢"

                    cards_html += f"""
                    <div style="background:{bg_c};border:1px solid {border_c};border-left:4px solid {border_c};
                        border-radius:4px;padding:10px 14px;min-width:148px;">
                      <div style="font-family:'Barlow Condensed',sans-serif;font-size:20px;font-weight:700;
                          color:#ffd600;letter-spacing:1px;">{strike}</div>
                      <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#7fa8c8;margin-top:2px;">
                          {opt_dot} {opt} &nbsp;{money} &nbsp;{vs_avg}</div>
                      <div style="font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:700;
                          color:{bias_c};letter-spacing:1px;margin-top:5px;">{bias_icon}</div>
                      <div style="font-family:'JetBrains Mono',monospace;font-size:11px;
                          color:#3a6080;margin-top:4px;">&#x1F552; {t_str}</div>
                    </div>"""
                cards_html += "</div>"
                ...
                vs_fig.add_bar(x=oc_sorted_vs["Strike"], y=oc_sorted_vs["CE_Volume"]/1e3, name="CE Volume", marker_color="#ff3d57", opacity=0.75)
                vs_fig.add_bar(x=oc_sorted_vs["Strike"], y=oc_sorted_vs["PE_Volume"]/1e3, name="PE Volume", marker_color="#00e676", opacity=0.75)
                vs_fig.add_hline(y=avg_ce_vol*spike_threshold/1e3, line_color="#ff8c00", line_dash="dash", line_width=1.2,
                                  annotation_text=f"CE {spike_threshold:.0f}x threshold", annotation_font_color="#ff8c00", annotation_font_size=9)
                vs_fig.add_hline(y=avg_pe_vol*spike_threshold/1e3, line_color="#00d4ff", line_dash="dash", line_width=1.2,
                                  annotation_text=f"PE {spike_threshold:.0f}x threshold", annotation_font_color="#00d4ff", annotation_font_size=9)
                vs_fig.update_layout(barmode="group", height=280, paper_bgcolor="#070b0f", plot_bgcolor="#070b0f",
                                      font=dict(family="JetBrains Mono", color="#7fa8c8", size=11),
    ...
    # SIDEBAR
    # ======================================================
    st.sidebar.markdown("""<div style="font-family:'Barlow Condensed',sans-serif;font-size:16px;font-weight:700;
            letter-spacing:2px;color:#ff8c00;border-bottom:1px solid #1e3040;padding-bottom:8px;margin-bottom:12px;">
      EXECUTE TRADE</div>""", unsafe_allow_html=True)
    ...
    if not oc_sb.empty and sel_sb and spot_sb:
        ...
        st.sidebar.markdown(f"""<div style="background:#0d1117;border:1px solid {sb_color};border-radius:3px;
                    padding:10px;margin:8px 0;font-family:'JetBrains Mono',monospace;font-size:12px;">
          <div style="color:#7fa8c8;">LTP</div>
          <div style="color:{sb_color};font-size:20px;font-weight:700;">Rs{cur_price_sb:.2f}</div>
        ...
        if action_sb == "SELL":
            st.sidebar.markdown(f"""<div style="font-family:'Barlow Condensed',sans-serif;font-size:11px;color:#ff3d57;letter-spacing:1px;">
                EST. MAX RISK ~ Rs{cur_price_sb*2*lot_sb:,.0f}</div>""", unsafe_allow_html=True)
        if MARKET_OPEN:
            if st.sidebar.button("EXECUTE", use_container_width=True, type="primary"):
                ...
                if grand_sb <= st.session_state.daily_loss_limit:
                    ...
                    trade_sb = {"Entry_Time": datetime.now(IST).strftime("%H:%M:%S"), "Date": datetime.now().strftime("%Y-%m-%d"),
                                "Index": sel_sb, "Strike": strike_sb, "Type": opt_type_sb, "Action": action_sb,
            ...
            st.sidebar.markdown(f"""<div style="background:#1a0a00;border:1px solid #ff8c00;border-radius:3px;
            padding:8px 12px;font-family:'Barlow Condensed',sans-serif;font-size:13px;color:#ff8c00;letter-spacing:1px;">
            MARKET {session_label}</div>""", unsafe_allow_html=True)
    else:
        st.sidebar.info("Load option chain from Tab 1")

    st.sidebar.markdown("---")
    st.sidebar.markdown("""<div style="font-family:'Barlow Condensed',sans-serif;font-size:13px;font-weight:700;letter-spacing:2px;color:#7fa8c8;">RISK CONTROLS</div>""", unsafe_allow_html=True)
    new_limit_sb = st.sidebar.number_input("Daily Loss Limit (Rs)", value=int(st.session_state.daily_loss_limit), step=1000, key="dl_limit_sb")
    st.session_state.daily_loss_limit = new_limit_sb
    new_max_sb = st.sidebar.number_input("Max AI Positions", value=st.session_state.get("max_open_positions", 6), min_value=1, max_value=20, step=1, key="max_pos_sb")
    ...
    st.sidebar.markdown(f"""<div style="background:#0d1117;border:1px solid #1e3040;border-radius:3px;
            padding:10px;margin-top:8px;font-family:'JetBrains Mono',monospace;">
      <div style="font-family:'Barlow Condensed',sans-serif;font-size:10px;letter-spacing:1.5px;color:#7fa8c8;">Today P&L</div>
      <div style="font-size:22px;font-weight:700;color:{pnl_c_sb};">Rs{grand_sb:,.0f}</div>
      <div style="font-size:11px;color:#3a6080;">Open: Rs{open_pnl_sb:,.0f} | Closed: Rs{closed_pnl_sb:,.0f}</div>
      <div style="margin-top:6px;">
        <div style="font-size:10px;color:#3a6080;margin-bottom:2px;">vs Limit Rs{st.session_state.daily_loss_limit:,}</div>
    ...
    if st.session_state.alert_log:
        st.sidebar.markdown("---")
        st.sidebar.markdown("""<div style="font-family:'Barlow Condensed',sans-serif;font-size:11px;letter-spacing:1.5px;color:#ff8c00;">ALERTS</div>""", unsafe_allow_html=True)
        for al in st.session_state.alert_log[-5:][::-1]:
            al_c = "#ff3d57" if al["type"] == "DANGER" else "#00d4ff"
            st.sidebar.markdown(f"""<div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:{al_c};padding:2px 0;">{al['time']} {al['msg']}</div>""", unsafe_allow_html=True)